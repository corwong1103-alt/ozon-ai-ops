  ...
  return (
    <AppShell title="任务记录" eyebrow="Operations Log" user={user}>
      <div className="ledger-card overflow-hidden">
        ...
        {tasks.map((task) => (
          <div key={task.id} className="grid grid-cols-12 gap-3 border-b border-line px-4 py-4 last:border-b-0">
            <div className="col-span-12 font-bold md:col-span-2">{typeLabels[task.type] ?? task.type}</div>
            <div className="col-span-6 text-sm text-steel md:col-span-2">{task.status}</div>
