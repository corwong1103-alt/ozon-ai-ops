...
## 19. 2026-06-17 Apify Ozon Market Search 接通记录

### 背景

Ozon 全站市场搜索此前不再继续走 `composer-api.bx`、Selenium、Playwright 或 GitHub parser 路线。已验证 Ozon 前台接口存在反爬挑战，最终采用 Apify 作为 OzonAI 的 Ozon Market Data Provider。

最终选定 Actor：

```text
Actor 名称：Ozon.ru Product Scraper PRO | Парсер товаров Ozon
Actor ID：zen-studio/ozon-scraper-pro
Provider：Apify
用途：关键词搜索、类目搜索、商品详情、图片、价格、评分、评论数、卖家信息
```

### 本轮发现的问题

1. `/integrations` 配置页测试按钮可以启动 Apify Actor，但页面曾一直显示“测试中”。
2. 根因是旧实现使用 `run-sync-get-dataset-items` 长连接等待 Actor 完成；Actor 运行时间超过前端等待时，按钮不会及时返回。
3. 改造后采用“启动 Actor → 获取 Run ID / Dataset ID → 轮询状态 → 30 秒超时 → 读取 partial Dataset → 必要时 abort”的流程。
4. `/research/ozon` 初版仍显示“这里暂时不会放假数据”，原因是搜索页没有使用与配置测试一致的 partial Dataset 流程，并且 normalizer 未覆盖 Apify 实际字段。
5. Apify 实际返回字段包括：
...
- `lib/services/ozon-market.ts`
  - 接入 Apify Actor 启动 API。
  - 不再依赖 `run-sync-get-dataset-items`。
  - 新增 Run 轮询。
  - 新增 30 秒超时。
  - 超时前读取 partial Dataset。
  - 超时后尝试 abort。
  - 搜索页与测试页统一使用真实 Apify 返回数据。
  - 新增搜索日志：
...
- `lib/ozon-market-normalizer.ts`
  - 新增 Apify Ozon 商品字段归一化。
  - 支持真实字段：`coverImageUrl`、`coverImage`、`cardPriceDecimal`、`originalPriceDecimal`、`productLink`、nested `rating`、nested `price.cardPrice.price`。
  - 增加价格递归兜底。
  - 搜索结果优先展示有图、有价格、有评分的商品。

- `/integrations`
  - Apify Ozon Market 支持 Token + Actor ID。
  - 测试按钮最多等待 30 秒。
  - 页面直接显示调试 JSON。
  - 后台日志输出 Apify 请求、Run ID、Dataset ID、状态和 partial Dataset。

- `/research/ozon`
  - `mode=market` 已确认进入 Apify 查询流程。
  - 搜索关键词会调用 `searchOzonMarketProducts()`，内部调用 `searchProducts()` / `searchCategory()`。
  - 页面直接展示真实商品卡片。
  - 商品卡片字段包括：
...
由于 Ozon 为俄语站，部分英文关键词命中较弱。本轮加入轻量关键词映射：
...
用户仍可在页面输入英文，后端会自动用更适合 Ozon 的俄语关键词查询。
...
```text
/research/ozon?mode=market&keyword=backpack
/research/ozon?mode=market&keyword=phone%20case
/research/ozon?mode=market&keyword=pet%20toy
```
...
cards=12
empty=0
first=Рюкзак HV379 черный, 28х12х40 см
price=494
...
cards=18
empty=0
first=Водонепроницаемый чехол для телефона
price=138
...
cards=16
empty=0
first=Интерактивная игрушка для кошек, умный мячик с хвостиком, дразнилка для котят
price=168
```
...
```text
[ozon_market_search] {"keyword":"backpack","categoryId":"","count":6,"firstTitle":"Рюкзак HV379 черный, 28х12х40 см","firstImage":"https://ir.ozone.ru/s3/multimedia-1-z/10675173995.jpg","firstPrice":494}

[ozon_market_search] {"keyword":"phone case","categoryId":"","count":9,"firstTitle":"Водонепроницаемый чехол для телефона","firstImage":"https://ir.ozone.ru/s3/multimedia-1-4/10896594700.jpg","firstPrice":138}

[ozon_market_search] {"keyword":"pet toy","categoryId":"","count":8,"firstTitle":"Интерактивная игрушка для кошек, умный мячик с хвостиком, дразнилка для котят","firstImage":"https://ir.ozone.ru/s3/multimedia-1-q/7509934862.jpg","firstPrice":168}
```

结论：

```text
Ozon 全站 Market Search 已真正接通。
三组关键词返回不同真实商品。
页面不再显示“这里暂时不会放假数据”。
商品图、标题、价格、评分、评论数、卖家字段均来自 Apify/Ozon 真实返回。
```

### 当前仍需注意

- Apify Actor 不是秒级接口，通常需要 20-30 秒以上才写入 partial Dataset。
- 当前为了页面可用性，搜索页采用 30 秒 partial Dataset 策略；这意味着结果数量可能不是完整搜索结果，但已经可用于调研和入池。
- 如果后续希望完整 Top 20/Top 50，应改为后台任务模式：创建搜索任务，前端轮询 TaskLog，不阻塞页面请求。
- Apify 是按事件/结果计费，频繁刷新同一关键词会消耗额度。
...
lib/services/ozon-market.ts
lib/ozon-market-normalizer.ts
tests/ozon-market.test.mjs
app/integrations/actions.ts
components/IntegrationFeedbackForms.tsx
app/globals.css
docs/OzonAI-total-project-log-2026-06-17.md
```
...
unit tests: 7 passed
build: passed
线上 /research/ozon 三关键词验证 passed
```
